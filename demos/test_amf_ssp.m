...
%
tic
d_bp=amf_bandpass(d2dssp_nmo,dt,flo,fhi,nplo,nphi,phase,verb0);
toc
%
...
%
tic
d_bpsosvmf=amf_bandpasssosvmf(d2dssp_nmo,dt,flo,fhi,nplo,nphi,phase,verb0,niter,liter,order1,eps_dv,eps_cg,tol_cg,rect,verb1,adj,add,n1,n2,ns,order2,eps,ndn,nds,type_mf,ifsmooth);
toc
%
...
% 
tic
d_bpsosvmffkcurvelet=amf_bandpasssosvmffkcurvelet(d2dssp_nmo,dt,flo,fhi,nplo,nphi,phase,verb0,niter,liter,order1,eps_dv,eps_cg,tol_cg,rect,verb1,adj,add,n1,n2,ns,order2,eps,ndn,nds,type_mf,ifsmooth,w,c1,c2,c3,niter1);
toc
%
%% AMF
% dt=0.0005; % sampling
...
subplot(3,2,2); imagesc(1:nh,t,reshape(d_bp,n1,nh)); hold on;
...
subplot(3,2,3); imagesc(1:nh,t,reshape(d_bpsosvmf,n1,nh)); hold on;
...
annotation(gcf,'textbox',...
    ...
    'BackgroundColor',[1 1 1]);
%
subplot(3,2,4); imagesc(1:nh,t,reshape(d_bpsosvmffkcurvelet,n1,nh)); hold on;
text(-3.5,-75,'(d)','color','k','Fontsize',22,'fontweight','bold','HorizontalAlignment','center');
plot(1:nh,t410_ref*ones(1,nh),'--r','linewidth',4)
plot(1:nx*ny,t660_ref*ones(1,nx*ny),'--r','linewidth',4)
title('BP+SOSVMF+Curvelet')
colormap(amf_seis);caxis([-0.02 0.02]); 
axis xy
xlim([1 35])
ylim([-300 -100])
...
print(gcf,'-depsc','-r300','fig19.eps');
