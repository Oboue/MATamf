...
%
tic
d_bpsosvmf_z=amf_bandpasssosvmf(d_z,t,flo,fhi,nplo,nphi,phase,verb0,niter,liter,order1,eps_dv,eps_cg,tol_cg,rect,verb1,adj,add,n1,n2,ns,order2,eps,ndn,nds,type_mf,ifsmooth);
toc
%
...
tic
d_bpsosvmf_r=amf_bandpasssosvmf(d_r,t,flo,fhi,nplo,nphi,phase,verb0,niter,liter,order1,eps_dv,eps_cg,tol_cg,rect,verb1,adj,add,n1,n2,ns,order2,eps,ndn,nds,type_mf,ifsmooth);
toc
%
%% Denosing using the BP+SOSVMF method 
% Parameter tuning：add the key parameters of the SOSVMF method
...
% 
tic
d_bpsosvmffkcurvelet_z=amf_bandpasssosvmffkcurvelet(d_z,t,flo,fhi,nplo,nphi,phase,verb0,niter,liter,order1,eps_dv,eps_cg,tol_cg,rect,verb1,adj,add,n1,n2,ns,order2,eps,ndn,nds,type_mf,ifsmooth,w,c1,c2,c3,niter1);
toc
...
% 
tic
d_bpsosvmffkcurvelet_r=amf_bandpasssosvmffkcurvelet(d_r,t,flo,fhi,nplo,nphi,phase,verb0,niter,liter,order1,eps_dv,eps_cg,tol_cg,rect,verb1,adj,add,n1,n2,ns,order2,eps,ndn,nds,type_mf,ifsmooth,w,c1,c2,c3,niter1);
toc
%% AMF
% dt=0.0005; % sampling
par.dt=t; % sampling
...
% BP+SOSVMF+Curvelet
    ...
    for n=1:size(d_bpsosvmffkcurvelet_r,2)
        R=d_bpsosvmffkcurvelet_r(:,n);
        Z=d_bpsosvmffkcurvelet_z(:,n);
        [itr,itrms] = amf_makeRFitdecon_la_norm(R,Z,dt,nt,ph,gauss,itmax,minderr);
        sub(n).itrbpsosvmfcurvelet=itr';
    end
...
annotation(gcf,'textarrow',[0.795555555555555 0.764444444444444],...
  ...
  %%  
    ...
    title('Vertical (raw)')
    subplot(3,2,2)
    amf_wigb(d_bpsosvmffkcurvelet_z,2,h,t)
    xlabel('Distance (km)')
    ylabel('Time (s)')
    set(gca,'fontsize',16)
    ...
    title('Radial (raw)')
    subplot(3,2,4)
    amf_wigb(d_bpsosvmffkcurvelet_r,2,h,t)
    xlabel('Distance (km)')
    ylabel('Time (s)')
    set(gca,'fontsize',16)
...
annotation(gcf,'arrow',[0.884444444444444 0.851111111111111],...
    ...
    title('RF (raw)')
   subplot(3,2,6)
    amf_wigb([sub.itrbpsosvmfcurvelet],4,h,t)
    xlabel('Distance (km)')
    ylabel('Time (s)')
    ylim([-5 30])
    set(gca,'fontsize',16)
...
annotation(gcf,'textarrow',[0.304444444444444 0.286666666666666],...
    ...
    subplot(3,2,4)
    imagesc(dist0,t,[sub.itrbpsosvmfcurvelet]);
    colormap(amf_seis);
    caxis([-0.4 0.4])
set(gca,'Linewidth',2,'Fontsize',16,'Fontweight','bold');
    title('RF (BP+SOSVMF+Curvelet)')
