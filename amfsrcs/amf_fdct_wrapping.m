function C = amf_fdct_wrapping(x, is_real, finest, nbscales, nbangles_coarse)
...
if finest == 1,
    ...
    [wl_1,wr_1] = amf_fdct_wrapping_window(coord_1);
    [wl_2,wr_2] = amf_fdct_wrapping_window(coord_2);
...
for j = scales,
    ...
    for quadrant = 1:nbquadrants
        ...
        wl_left = amf_fdct_wrapping_window(coord_corner);
        [wl_right,wr_right] = amf_fdct_wrapping_window(coord_right);
        ...
        for subl = 2:(nbangles_perquad-1);
            ...
            coord_right = 1/2 + floor(4*M_vert)/(wedge_endpoints(subl+1) - wedge_endpoints(subl)) * ...
                (wrapped_XX - mid_line_right)./(floor(4*M_vert)+1 - wrapped_YY);
            wl_left = amf_fdct_wrapping_window(coord_left);
            [wl_right,wr_right] = amf_fdct_wrapping_window(coord_right);
            wrapped_data = wrapped_data .* (wl_left .* wr_right);
            switch is_real
                case 0
        ...
        wl_left = amf_fdct_wrapping_window(coord_left);
        [wl_right,wr_right] = amf_fdct_wrapping_window(coord_corner);
