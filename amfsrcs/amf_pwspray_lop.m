function [u1,u] = amf_pwspray_lop(adj,add,n,nu,u1,u,dip,nr,nt,nx,order,eps)
%amf_pwspray_lop: Plane-wave spray operator(benchmarked with Madagascar, exact
% the same) 
% 
...
[ u1,u ] = amf_adjnull( adj,add,n,nu,u1,u );
...
for i=0:n2-1
    ...
    if adj
        ...
        for is=ns-1:-1:0
          ...
          [w,diag,offd,trace] = amf_predict_step(e,nw,1,1,n1,p(:,ip),trace); 
        end
       ...
       for is=0:ns-1
           ...
           j=ip*ns2+ns-is-1;
           [w,diag,offd,trace] = amf_predict_step(e,nw,0,0,n1,p(:,ip+1),trace);
           for i1=0:n1-1
       ...
       for is=0:ns-1
           ...
           j=ip*ns2+ns+is+1;
           [w,diag,offd,trace] = amf_predict_step(e,nw,0,1,n1,p(:,ip),trace);
           for i1=0:n1-1
              u(j*n1+i1+1)= u(j*n1+i1+1)+trace(i1+1);
           end
       end 
